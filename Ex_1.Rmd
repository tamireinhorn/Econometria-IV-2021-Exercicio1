...
clean_df <- df %>% 
    select(Asset, Date, Time, Close, Volume) %>%
...
measures <- clean_data %>% 
  ...
  summarise(daily=sum(r_cc,na.rm=T), RV = sum(r_cc^2, na.rm = T), VOL = sum(Volume)) %>% #for each stock and day we obtain three variables: daily, which is the accumulated closed-to-closed returns, RV, which is the accumulated SQUARED closed-to-closed returns, and VOL, which sums the volumes of the minutes of that day. 
...
measures_wide <- measures %>% select(Stock, Date, RV) %>% dcast(Date ~ Stock, value.var = 'RV')

datatable(cbind(Date = rv_wide[,1], round(RV_wide[,1:30],5)), #we round the numbers to contain 5 digits
          
          width = "100%", 
          caption = 'Daily Realized Volatility',
          options = list(scrollX = T))

#we want it to be 30 (assets) and 4310 days
dim(measures_wide) 

#we want to check missing values per stock 
measures_na <- measures %>%
  group_by(Stock) %>%
  summarise (na_stock=sum(is.na)) %>%
  ungroup()
```

3.3 HISTOGRAMS checar

```{r}
measures %>% 
  group_by(Asset, Date) %>% 
  mutate(RV = 100*min(RV, 0.001)) %>%
  nest(data = c(RV, Volume, Date)) %>% 
