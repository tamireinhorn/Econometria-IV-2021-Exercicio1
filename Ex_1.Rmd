...
make_data <- function(ticker, flag = 0){
  ...
  dias <- unique(AXP$Date)
  n <- length(dias)
  arr <- array(NA, dim = c(390, 7, n))
  for(j in 1:n){
    arr[,,j] <- clean_ticker[clean_ticker$Date == dias[j],]
  }
  return(arr)
}
AXP2 <- clean_data(AXP)
minute_df <- data.frame('Time' = unique(AXP2$Time))
df1 <- AXP2[AXP2$Date == AXP$Date[1],]

df2 <- left_join(minute_df, df1, by = 'Time')
```

```{r}
##Construa a matriz prum dia específico:
a <- make_data(AXP)
