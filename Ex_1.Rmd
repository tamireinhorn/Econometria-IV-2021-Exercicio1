...
make_data <- function(ticker, flag = 0){
 ...
 ## arr <- array(NA, dim = c(390, dim(clean_ticker)[2], n)) deprecated
  ...
  for(j in 1:n){
    ...
    df2 <- left_join(minute_df, df1, by = 'Time')
    list[[j]] <- vector(mode = 'list', length = 2) ##Creates the inner list
    list[[j]][1] <- as.list(dias[j]) ##Fills the day. As list stops conversion to int.
    list[[j]][[2]] <- df2
  }
  return(list)
}
...
j = 1 
list <- vector(mode = "list", length =  n)
arr <- array(NA, dim = c(390, dim(clean_ticker)[2], n))
df1 <- clean_ticker[clean_ticker$Date == dias[j],]
df2 <- left_join(minute_df, df1, by = 'Time')

list[[j]] <- vector(mode = 'list', length = 2)
list[[j]][1] <- as.list(dias[j])
list[[j]][[2]] <- df2
