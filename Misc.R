...
RollingWindow <- function(key_stock = 'SPY', window_size = 1000, week = 5, month = 22, df, model_type = 'level'){
  ...
  day_list <- sort(unique(key_stock_df$Date))
  length_rw <- days - (month - 1) - 1 
  #length_df would be the number of dates for which RV_m, RV_w and RV_t will be constructed, to be used in all windows eventually estimated. We eliminate the first 21 dates, and the last one, because the last one 
  ...
  for(i in 1:length_rw){
    ...
    RV_m[[i]] <- (1/month) * sum(key_stock_df$RV[i:(i+21)]) #for i=1 (index of the first OBS in the original SPY data for which RV_t RV_m and RV_w are constructed), we sum over observations 1 to 22 of ORIGINAL spy data, yielding the first observation for our regressor
    RV_w[[i]] <- (1/week) * sum(key_stock_df$RV[(i+17):(i+21)]) #for i=1, we sum over observations 18 to 22
    RV_t[[i]] <- key_stock_df$RV[(i+21)] #for i=1, we select observation 22
    current_date[[i]] <- key_stock_df$Date[(i+21)]
    future_date[[i]] <- key_stock_df$Date[(i+21+1)] #if the code works, the last observation of future_date will be the last date for which SPY has an observation. Testei e ta ok
  }
  ...
  #renomear RV em SPY por dependent
