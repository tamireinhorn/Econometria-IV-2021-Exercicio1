...
remotes::install_github("gabrielrvsc/HDeconometrics") ##This package does EXACTLY what we need! This installs it from outside CRAN
...
x_var <- data.matrix(df4[,!names(df4) %in% c('Date', 'VOL', 'RV', 'past_date', 'V', 'daily')])
# Setting the range of lambda values
lambda_seq <- 10^seq(2, -2, by = -.1)
# Using glmnet function to build the ridge regression in r
fit <- tidy(glmnet(x_var, y_var, alpha = 0, lambda  = lambda_seq))
# Checking the model
f <- tidy(fit)
for(l in lambdas_seq){
  print(lambdas_seq[[l]])
}
```
...
##The key_stock argument defines the stock over which we want to model. It defaults to SPY since it is the ETF.
  ...
  i <- 1
  ###This cannot go on until days, this needs to stop at the last possible window. ##days - window_size +1 maybe
    ...
    y_var <- data.matrix(scale(full_data$RV, center = TRUE, scale = FALSE)) ##GLMNET only works with data matrix, so we need to convert it and separate the variables. Ridge Regression also imposes that the response is centered.
    ...
    lambda_sequence <- 10^seq(2, -2, by = -.1)
    X_scaled <- scale(x_var)
    aic <- c()
    bic <- c()
    for (lambda in 1:length(lambda_sequence)) {
      # Run model
      ridge <- glmnet(x_var, y_var, alpha = 0, lambda = lambda_sequence[lambda], standardize = TRUE)
      # Extract coefficients and residuals (remove first row for the intercept)
      betas <- as.vector((as.matrix(coef(model))[-1, ]))
      resid <- y - (X_scaled %*% betas)
      # Compute hat-matrix and degrees of freedom
      ld <- lambda_sequence[lambda] * diag(ncol(X_scaled))
