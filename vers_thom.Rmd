...
autocorr_har_f <- function(df){
  ...
  #generating HAR models
  
  #Below we group the data by stock, and estimate the HAR model for each stock, using tidyverse`s feature group_map() and the HARestimate function.

har_est_rv <- df %>% 
  group_by(Stock) %>%
  group_map(.tbl=df$RV, .f = ~HAREstimate(RM = df$RV, periods = c(1,5,22)) )

har_est_log_rv <- df %>% 
  group_by(Stock) %>%
  group_map(.tbl=df$log_RV, .f = ~HAREstimate(RM = df$log_RV, periods = c(1,5,22)) )


#extracting the estimated coefficients and simulating theoretical processes:

har_sim_rv <- df %>% 
  group_by(Stock) %>%
  group_map(.tbl=df$RV, .f = ~HARSimulate(len=1500, periods = c(1, 5, 22), coef=har_est_rv, errorTermSD = 0.001) ) #checar se coef=har_est_rv imputa os coeficientes estimados acima para a stock em questão que estamos, ídem abaixo. Acho que o faz pois os próprios coeficientes foram estimados por stock.
    
har_sim_log_rv <- df %>% 
  group_by(Stock) %>%
  group_map(.tbl=df$log_RV, .f = ~HARSimulate(len=1500, periods = c(1, 5, 22), coef=har_est_log_rv, errorTermSD = 0.001) )
...
testaxp <- complete_daily_df %>% filter(Stock=="AXP")
