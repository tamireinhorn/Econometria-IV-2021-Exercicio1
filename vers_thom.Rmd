...
#test <- complete_daily_df %>%
 # filter(Stock=="AXP")
  #ggplot(test,aes(x=RV))+geom_histogram()
...
ggplot(complete_daily_df, aes(x = log_RV)) +
  geom_histogram() +
  labs(title = "log(RV) Histograms", x = "", y = "") +
  facet_wrap(~ Stock, ncol = 4) +
  theme_minimal()
```
...
ggplot(complete_daily_df, aes(x = Date, y= 100*RV)) +
  geom_line() +
...
```{r}
ggplot(complete_daily_df, aes(x = Date, y= 100*log_RV)) +
  geom_line() +
  facet_wrap(~ Stock, ncol = 2, scales = 'free') +
  labs(x = "", y = "", title = 'log_RV Time Series (in percentage terms, %)') +
  theme_bw() + theme(legend.position = "none")
```
...
log_RV_tsibble %>% ACF(RV, lag_max = 200) %>% autoplot() + theme_bw() + labs(x = 'Lags', y = '')
