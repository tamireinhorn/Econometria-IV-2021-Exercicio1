...
We will now conduct an estimation of a HAR Model for the realized volatility for each Stock in our sample.

```{R}

#First, let`s estimate for AXP
RV_tsibble_AXP <- RV_tsibble %>% 
  filter(Stock=="AXP") %>%
  select(Stock)

HAREstimate(RV_tsibble_AXP, BPV = NULL, RQ = NULL, periods = c(1,5,22),
