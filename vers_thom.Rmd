...
autocorr_f <- function(df){
  ...
  names(autocorr) <- c("stock_name")   #seq(0,29,1) indicate that there are 30 lags of estimated ACFs, for each Stock. The above function generates, thus, one dataframe listing ACFs, for each Stock.
  
  autocorr$autocorr_level = acf(complete_daily_df$RV, lag.max = 29, pl = F)[[1]] %>% as.numeric() 
  autocorr$autocorr_log = acf(complete_daily_df$log_RV, lag.max = 29, pl = F)[[1]] %>% as.numeric()     #[[1]] selects the the values of the ACFs themselves, as the function acf() generates other objects that are not of interest
  
  #ACFs for AR Model
  
  autocorr$autocorr_ar_level = ARMAacf(ar = arima(complete_daily_df$RV, order = c(1, 0, 0))$coef[1], ma = 0, lag.max = 29) %>% as.numeric()   
...
#The ARMAacf simulates a model for coefficients we give as imput, and returns the ACFs. @coef[1] selects only the auto-regressive parameter, and not the intercept.
  ...
  autocorr$autocorr_ar_log = ARMAacf(ar = arima(log(complete_daily_df$log_RV), order = c(1, 0, 0))$coef[1], ma = 0, lag.max = 29) %>% as.numeric()
...
long memory of empirical volatility seems be fully achieved by estimating HAR processes for actual data. If we focus on the period of time spanning our sample, approximately 17 years, allowing for less lags, there is a relatively linear decay in the figures of Corsi (2009) that indicate that the HAR model can be effective in reproducing actual voltatiliry.
...
for (i in 1:30) {
  ...
  #colocar número certo de dias (contar quantos dias cada stock tem?)
}
