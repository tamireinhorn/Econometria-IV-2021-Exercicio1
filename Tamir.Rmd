...
for(stock in daily_stock_list){
 plot <- ggplot(stock, aes(x = RV)) + geom_histogram(bins = 100) +labs(title = paste("RV Histogram for ", stock_name_v[stock],sep = ""))
 plot_list[stock] <- plot
}

daily_stock_list_1_df <- as.data.frame(daily_stock_list[1])

plot <- ggplot(daily_stock_list_1_df, aes(x = AXP.RV)) + geom_histogram(bins = 100) +labs(title = paste("RV Histogram for ", "AXP",sep = ""))
...
for(i in 1:length(daily_stock_list)){
  ...
  normal <- data.frame(dnorm(daily_stock_list[[i]]$RV, mean = mean(daily_stock_list[[i]]$RV), sd = sd(daily_stock_list[[i]]$RV)))
